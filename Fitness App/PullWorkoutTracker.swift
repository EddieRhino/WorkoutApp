...
struct PullWorkoutTracker: View {
...
struct PullWorkoutTracker_Previews: PreviewProvider {
    static var previews: some View {
        PullWorkoutTracker()
    }
}

func saveWorkoutsPull(_ workouts: [Exercise]){
    let encoder = JSONEncoder()
    if let encoded = try? encoder.encode(workouts) {
        UserDefaults.standard.set(encoded, forKey: "SavedWorkoutsPull")
    }
}

func loadWorkoutsPull() -> [Exercise] {
    if let savedData = UserDefaults.standard.data(forKey: "SavedWorkoutsPull"){
...
func saveWorkoutOptionsPull(_ workoutOptions: [String]){
    let encoder = JSONEncoder()
    if let encoded = try? encoder.encode(workoutOptions) {
        UserDefaults.standard.set(encoded, forKey: "SavedWorkoutOptionsPull")
    }
}

func loadWorkoutOptionsPull() -> [String] {
    if let savedData = UserDefaults.standard.data(forKey: "SavedWorkoutOptionsPull"){
        let decoder = JSONDecoder()
